...
生产环境由 systemd 托管 4 个 service（`db-driven-trading`、`realtime-kline-sync`、`sync-macd-indicators`、`kline-web`），另由 `service-health-check.timer` 每分钟触发一次健康检查。

```bash
# 安装/更新全部 service 与 timer（模板位于 deploy/systemd/*.service.in）
...
健康检查脚本 `scripts/service_health_check.py` 执行 4 项检查：

1. **服务存活**：`systemctl is-active` 检查上述 4 个 unit（非 systemd 环境自动跳过）
2. **K线新鲜度**：`klines_1m` 最新数据滞后不超过 `HEALTH_MAX_KLINE_AGE_SECONDS`（默认 180 秒）
3. **指标新鲜度**：`klines_1m_macd_smooth_ma` 同上
4. **Web 可达**：HTTP 访问 `127.0.0.1:{WEB_PORT}/`，任意 HTTP 响应（含 302/4xx）视为存活

告警行为：

- 每个检查项独立冷却（默认 600 秒，`HEALTH_ALERT_COOLDOWN_SECONDS`），异常项合并为一条飞书消息
- 发送失败不记录冷却，下轮重试；某项恢复时发送一次"已恢复"通知
- 全部正常时静默退出；脚本异常只记日志，退出码恒为 0
- 冷却状态目录优先级：`HEALTH_STATE_DIR` > `/run/service-health-check` > `data/.health_state/`
